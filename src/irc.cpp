...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("NixonCoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #NixonCoinTEST2\r");
            Send(hSocket, "WHO #NixonCoinTEST2\r");
        } else {
            // randomly join #NixonCoin00-#NixonCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #NixonCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #NixonCoin%02d\r", channel_number).c_str());
        }
