NixonCoin-qt: Qt4 GUI for NixonCoin
===============================
...
Alternatively, install Qt Creator and open the `NixonCoin-qt.pro` file.

An executable named `NixonCoin-qt` will be built.
...
A warning for people using the *static binary* version of NixonCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of NixonCoin is linked against libdb5.3.
...
installed causes NixonCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
